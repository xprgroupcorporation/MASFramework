...
- Source: `ServerScriptServiceAssets/ServerHandler/Services/SharedLoopServer.luau`
- Kind: Service
- Runtime: Server
- Module path: `ServerScriptServiceAssets/ServerHandler/Services/SharedLoopServer`
...
- Provide the `Shared Loop Server` implementation for the MAS Framework runtime.
...
- Original file: `D:/Main Work/XPR Group Corp. All Docs/MAS Framework - Github ver/Plugin/Ref_for_documentation/MAS_Export_MAIN-BRANCH/ServerScriptServiceAssets/ServerHandler/Services/SharedLoopServer.luau`
- Documentation file: `D:/Main Work/XPR Group Corp. All Docs/MAS Framework - Github ver/Code_Documentation/MAS_Export_MAIN-BRANCH/ServerScriptServiceAssets/ServerHandler/Services/SharedLoopServer.md`
...
────────────────────────────────────────────────────
📦 SharedLoopServer — Version 0.1.4 (Server) (Service — Foundational)
(Made for MAS Framework — Public Standard Ver: 2.0.0+)
────────────────────────────────────────────────────

For efficiently running multi loop tasks for the Server side.
...
🔁 How It Works:
- Shares a single "Heartbeat connection" on the Server, 
rather than creating multiple "RunService" loops.
- Shares a single "while wait(0.5)" loop" on the Server, 
...
🔧 Supported 2 Shared Loop:
- Heartbeat
- while wait(0.5)
Note: Just read the module to understand it. How it work? Functions?
Note2: We use 0.5 in server due to we update server more often so we think this is useful for you!
...
✅ Maintains predictable tick behavior for game systems (such as cooldowns, updates, status checks, etc.)
...
- 🔒 Server-side loop ticks are consistent and minimize scheduling issues.

⚠️ Limitations / Considerations:
- ❌ Shared timing means you "cannot control independent intervals" in exact decimal per module 
